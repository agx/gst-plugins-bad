...
gst_bcm_dec_chain (GstPad * pad, GstObject * parent, GstBuffer * buffer);

static GstStateChangeReturn
gst_bcm_dec_change_state (GstElement * element, GstStateChange transition);
...
static void bcmdec_set_framerate (GstBcmDec * filter, guint32 resolution);

static gboolean
bcmdec_format_change (GstBcmDec * filter, BC_PIC_INFO_BLOCK * pic_info);

static BC_STATUS gst_bcm_dec_cleanup (GstBcmDec * filter);

static gboolean bcmdec_start_recv_thread (GstBcmDec * bcmdec);

static GstClockTime
bcmdec_get_time_stamp (GstBcmDec * filter, guint32 pic_no,
    GstClockTime spes_time);

static gboolean bcmdec_process_play (GstBcmDec * filter);

static gboolean bcmdec_alloc_mem_buf_que_pool (GstBcmDec * filter);

static gboolean bcmdec_release_mem_buf_que_pool (GstBcmDec * filter);

static void
bcmdec_put_que_mem_buf (GstBcmDec * filter, GSTBUF_LIST * gst_queue_element);

static GSTBUF_LIST *bcmdec_get_que_mem_buf (GstBcmDec * filter);

static void
bcmdec_ins_buf (GstBcmDec * filter, GSTBUF_LIST * gst_queue_element);

static GSTBUF_LIST *bcmdec_rem_buf (GstBcmDec * filter);
...
static void bcmdec_process_flush_start (GstBcmDec * filter);

static void bcmdec_process_flush_stop (GstBcmDec * filter);

static gboolean bcmdec_mul_inst_cor (GstBcmDec * filter);

static BC_STATUS bcmdec_create_shmem (GstBcmDec * filter, int *shmem_id);

static BC_STATUS
bcmdec_get_shmem (GstBcmDec * filter, int shmid, gboolean newsh);

static BC_STATUS bcmdec_del_shmem (GstBcmDec * filter);
...
static GSTBUF_LIST *bcmdec_rem_padbuf (GstBcmDec * filter);
...
static GstFlowReturn
bcmdec_send_buff_detect_error (GstBcmDec * bcmdec, GstBuffer * buf,
    guint8 * pbuffer, guint32 size,
    guint32 offset, GstClockTime tCurrent, guint8 flags)
{
 ...
 */
GstStaticPadTemplate sink_factory_bcm70015 =
    GST_STATIC_PAD_TEMPLATE ("sink", GST_PAD_SINK, GST_PAD_ALWAYS,
    GST_STATIC_CAPS ("video/mpeg, " "mpegversion = (int) {2, 4},"
        "systemstream =(boolean) false; " "video/x-h264;" "video/x-vc1;"
        "video/x-wmv, " "wmvversion = (int) {3};" "video/x-msmpeg, "
        "msmpegversion = (int) {43};" "video/x-divx, "
        "divxversion = (int) {3, 4, 5};" "video/x-xvid;"));

GstStaticPadTemplate sink_factory_bcm70012 =
    GST_STATIC_PAD_TEMPLATE ("sink", GST_PAD_SINK, GST_PAD_ALWAYS,
    GST_STATIC_CAPS ("video/mpeg, " "mpegversion = (int) {2},"
        "systemstream =(boolean) false; " "video/x-h264;" "video/x-vc1;"
...
static GstStaticPadTemplate src_factory =
    ...
    GST_STATIC_CAPS ("video/x-raw, " "format = (string) { YUY2 } , "
        "framerate = (fraction) [0,MAX], " "width = (int) [1,MAX], "
        "height = (int) [1,MAX]; " "video/x-raw, "
        "format = (string) { UYVY } , " "framerate = (fraction) [0,MAX], "
        "width = (int) [1,MAX], " "height = (int) [1,MAX]; "));
...
gst_bcm_dec_class_init (GstBcmDecClass * klass)
{
  ...
  gst_element_class_set_metadata (element_class, "Codec/Decoder/Video",
...
gst_bcm_dec_init (GstBcmDec * bcmdec)
{
  ...
  if (hwCaps.DecCaps & BC_DEC_FLAGS_M4P2) {
    bcmdec->sinkpad =
        gst_pad_new_from_static_template (&sink_factory_bcm70015, "sink");
  } else
    bcmdec->sinkpad =
    ...
    if (G_VALUE_TYPE (g_value) == GST_TYPE_BUFFER) {
      if (!strcmp ("video/x-h264", mime)) {
        ...
        if (info.size == 4) {
          ...
          memcpy (bcmdec->codec_params.sps_pps_buf, info.data, 4);
          ...
          for (index = 0; index < info.size; index++) {
            info.data += index;
            if (((info.size - index) >= 4) && (*info.data == 0x00)
                && (*(info.data + 1) == 0x00) && (*(info.data + 2) == 0x01)
                && (*(info.data + 3) == 0x0f)) {
              GST_DEBUG_OBJECT (bcmdec,
                  "VC1 Sequence Header Found for Adv Profile");
    ...
    if (!strcmp ("video/x-h264", mime)) {
      ...
      if (!strcmp ("video/x-wmv", mime)) {
        if (gst_structure_has_field (structure, "format")) {
          ...
          if (!g_strcmp0 (fourcc, "WVC1") || !g_strcmp0 (fourcc, "WMVA")) {
            bcmdec->input_format = BC_MSUBTYPE_VC1;
            GST_DEBUG_OBJECT (bcmdec, "InFmt VC-1 (AP)");
          } else {
            GST_DEBUG_OBJECT (bcmdec,
                "no codec_data. Don't know how to handle");
            goto out;
          }
        }
 ...
 */
static GstFlowReturn
gst_bcm_dec_chain (GstPad * pad, GstObject * parent, GstBuffer * buf)
{
  GstBcmDec *bcmdec;
  GstMapInfo info;
  guint32 offset = 0;
  ...
  if (GST_FLOW_OK != bcmdec_send_buff_detect_error (bcmdec, buf, pbuffer, size,
          offset, tCurrent, bcmdec->proc_in_flags)) {
    ret = GST_FLOW_ERROR;
    goto unmapout;
  }
#ifdef FILE_DUMP__
  bytes_written =
      fwrite (GST_BUFFER_DATA (buf), sizeof (unsigned char),
...
bcmdec_negotiate_format (GstBcmDec * bcmdec)
{
  ...
  gst_video_info_set_format (&info,
      vidFmt, bcmdec->output_params.width, bcmdec->output_params.height);
  info.fps_n = num;
  info.fps_d = den;
  ...
  if (bcmdec->output_params.clr_space == MODE422_YUY2) {
    ...
    bcmdec->output_params.y_size =
        bcmdec->output_params.width * bcmdec->output_params.height;
    bcmdec->output_params.uv_size =
        bcmdec->output_params.width * bcmdec->output_params.height / 2;
#ifdef YV12__
    if (bcmdec->interlace) {
      GST_DEBUG_OBJECT (bcmdec, "bcmdec_negotiate_format Interlaced");
      bcmdec->output_params.y_size =
  ...
  if (framerate_value != NULL) {
    ...
    GST_DEBUG_OBJECT (bcmdec, "framerate = %f rate_num %d rate_den %d",
        bcmdec->output_params.framerate, num, den);
  } else {
...
static gboolean
bcmdec_process_play (GstBcmDec * bcmdec)
{
  BC_STATUS sts = BC_STS_SUCCESS;
  ...
  if (bcInputFormat.mSubtype == BC_MSUBTYPE_WMV3
      || bcInputFormat.mSubtype == BC_MSUBTYPE_DIVX311) {
    //VC-1 Simple/Main
...
gst_bcm_dec_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      ...
      if (bcmdec_mul_inst_cor (bcmdec)) {
        ...
        if (sts == BC_STS_SUCCESS) {
          ...
          GST_ERROR_OBJECT (bcmdec,
              "dev open failed...ret GST_STATE_CHANGE_FAILURE");
    ...
    case GST_STATE_CHANGE_PAUSED_TO_READY:
      ...
      if (bcmdec->get_rbuf_thread) {
        ...
        GST_DEBUG_OBJECT (bcmdec, "get_rbuf_thread exit - %d errno = %d", ret,
            errno);
      ...
      if (bcmdec->push_thread) {
        ...
        GST_DEBUG_OBJECT (bcmdec, "push_thread exit - %d errno = %d", ret,
            errno);
  ...
  result =
      GST_ELEMENT_CLASS (gst_bcm_dec_parent_class)->change_state (element,
      transition);
...
bcmdec_get_buffer (GstBcmDec * bcmdec, gint size, GstBuffer ** obuf)
{
  ...
  *obuf = gst_buffer_new_allocate (NULL, size, NULL);

  if (obuf == NULL) {
    GST_ERROR_OBJECT (bcmdec, "gst_pad_alloc_buffer_and_set_caps failed %d ",
        ret);
    return FALSE;
  }
#if 0
...
static void
bcmdec_init_procout (GstBcmDec * bcmdec, BC_DTS_PROC_OUT * pout, guint8 * buf)
{
  ...
  //if (bcmdec->format_reset)
  {
    ...
    if ((bcmdec->output_params.stride) || (bcmdec->interlace)) {
      ...
      if (bcmdec->interlace)
        pout->StrideSz =
            bcmdec->output_params.width + 2 * bcmdec->output_params.stride;
      else
...
//      bcmdec->interlace = FALSE;
  ...
  if ((framerate) && (bcmdec->output_params.framerate != framerate)) {
    ...
    bcmdec->frame_time =
        (GstClockTime) (UNITS / bcmdec->output_params.framerate);
...
static void
bcmdec_set_aspect_ratio (GstBcmDec * bcmdec, BC_PIC_INFO_BLOCK * pic_info)
{
  switch (pic_info->aspect_ratio) {
    ...
    case vdecAspectRatioOther:
      bcmdec->output_params.aspectratio_x =
          pic_info->custom_aspect_ratio_width_height & 0x0000ffff;
      bcmdec->output_params.aspectratio_y =
          pic_info->custom_aspect_ratio_width_height >> 16;
      break;
  ...
  GST_DEBUG_OBJECT (bcmdec, "dec_par x = %d",
      bcmdec->output_params.aspectratio_x);
  GST_DEBUG_OBJECT (bcmdec, "dec_par y = %d",
      bcmdec->output_params.aspectratio_y);
}
...
static int
bcmdec_wait_for_event (GstBcmDec * bcmdec)
{
  int ret = 0, i = 0;
...
static void
bcmdec_flush_gstbuf_queue (GstBcmDec * bcmdec)
{
        ...
        if (gst_queue_element) {
          if (gst_queue_element->gstbuf) {
            ...
            if (result != GST_FLOW_OK) {
              GST_DEBUG_OBJECT (bcmdec, "exiting, failed to push sts = %d",
                  result);
              gst_buffer_unref (gst_queue_element->gstbuf);
              done = TRUE;
            } else {
              GST_DEBUG_OBJECT (bcmdec, "PUSHED, Qcnt:%d, Rcnt:%d",
                  bcmdec->gst_que_cnt, bcmdec->gst_padbuf_que_cnt);
              if ((g_inst_sts->rendered_frames++ > THUMBNAIL_FRAMES)
    ...
    while (bcmdec->streaming && !bcmdec->last_picture_set) {
      ...
      switch (sts) {
        case BC_STS_FMT_CHANGE:
          ...
          GST_DEBUG_OBJECT (bcmdec,
              "format change wait for rbuf thread start wait_cnt:%d", wait_cnt);

          break;
        case BC_STS_SUCCESS:
          if (!(pout.PoutFlags & BC_POUT_FLAGS_PIB_VALID)) {
            if (!bcmdec->silent)
              GST_DEBUG_OBJECT (bcmdec, "procout ret PIB miss %d",
          ...
          if (!bcmdec->silent)
            GST_DEBUG_OBJECT (bcmdec, "pic_number from HW is %u",
          ...
          if (bcmdec->prev_pic + 1 < pic_number) {
            if (!bcmdec->silent)
              GST_DEBUG_OBJECT (bcmdec, "LOST PICTURE pic_no = %d, prev = %d",
                  pic_number, bcmdec->prev_pic);
          }

          if (!bcmdec->interlace || bcmdec->sec_field) {
            GST_DEBUG_OBJECT (bcmdec, "Progressive or Second Field");
            GST_BUFFER_OFFSET (gstbuf) = 0;
            GST_BUFFER_TIMESTAMP (gstbuf) =
                bcmdec_get_time_stamp (bcmdec, pic_number,
                pout.PicInfo.timeStamp);
            ...
            if (bcmdec->gst_clock) {
              ...
              bcmdec->cur_stream_time =
                  cur_stream_time_diff + bcmdec->base_clock_time;
              bcmdec->prev_clock_time = clock_time;
        ...
        case BC_STS_TIMEOUT:
          GST_DEBUG_OBJECT (bcmdec,
              "procout timeout QCnt:%d, RCnt:%d, Paused:%d",
        ...
        case BC_STS_NO_DATA:
          ...
          break;
        default:
          GST_DEBUG_OBJECT (bcmdec, "unhandled status from Procout sts %d",
              sts);
          if (gstbuf) {
            gst_buffer_unref (gstbuf);
            gstbuf = NULL;
...
static GstClockTime
bcmdec_get_time_stamp (GstBcmDec * bcmdec, guint32 pic_no,
    GstClockTime spes_time)
{
  ...
  if (bcmdec->enable_spes) {
    if (spes_time) {
      ...
      if (bcmdec->spes_frame_cnt && bcmdec->last_output_spes_time) {
        bcmdec->frame_time =
            (time_stamp -
            bcmdec->last_output_spes_time) / bcmdec->spes_frame_cnt;
        bcmdec->spes_frame_cnt = 0;
      }
...
static void
bcmdec_process_flush_stop (GstBcmDec * bcmdec)
{
...
static void
bcmdec_process_flush_start (GstBcmDec * bcmdec)
{
  gint ret = 1;
  BC_STATUS sts = BC_STS_SUCCESS;
  struct timespec ts;
...
static BC_STATUS
gst_bcm_dec_cleanup (GstBcmDec * bcmdec)
{
  BC_STATUS sts = BC_STS_SUCCESS;
...
static void
bcmdec_put_que_mem_buf (GstBcmDec * bcmdec, GSTBUF_LIST * gst_queue_element)
{
...
static GSTBUF_LIST *
bcmdec_get_que_mem_buf (GstBcmDec * bcmdec)
{
  GSTBUF_LIST *gst_queue_element = NULL;
...
static gboolean
bcmdec_alloc_mem_buf_que_pool (GstBcmDec * bcmdec)
{
  GSTBUF_LIST *gst_queue_element = NULL;
  guint i = 0;
...
static gboolean
bcmdec_release_mem_buf_que_pool (GstBcmDec * bcmdec)
{
  GSTBUF_LIST *gst_queue_element;
  guint i = 0;
...
static void
bcmdec_ins_buf (GstBcmDec * bcmdec, GSTBUF_LIST * gst_queue_element)
{
  pthread_mutex_lock (&bcmdec->gst_buf_que_lock);
...
static GSTBUF_LIST *
bcmdec_rem_buf (GstBcmDec * bcmdec)
{
  GSTBUF_LIST *temp;
...
static BC_STATUS
bcmdec_insert_sps_pps (GstBcmDec * bcmdec, GstMapInfo * info)
{
  BC_STATUS sts = BC_STS_SUCCESS;
  ...
  for (i = 0; i < num_sps; i++) {
    ...
    memcpy (bcmdec->codec_params.sps_pps_buf + bcmdec->codec_params.pps_size,
        data, nal_size);
...
static gboolean
bcmdec_mul_inst_cor (GstBcmDec * bcmdec)
{
  struct timespec ts;
  gint ret = 0;
  int i = 0;
  ...
  if (g_inst_sts->cur_decode == PLAYBACK) {
    GST_DEBUG_OBJECT (bcmdec, "mul_inst_cor : ret false %d",
        g_inst_sts->cur_decode);
    return FALSE;
  }
    ...
    if (ret < 0) {
      if (errno == ETIMEDOUT) {
        if (g_inst_sts->cur_decode == PLAYBACK) {
          ...
          GST_DEBUG_OBJECT (bcmdec,
              "mul_inst_cor :wait for thumb nail decode finish");
          continue;
        }
      } else if (errno == EINTR) {
        return FALSE;
  ...
  GST_DEBUG_OBJECT (bcmdec, "mul_inst_cor : ret false cur_dec = %d wait = %d",
      g_inst_sts->cur_decode, g_inst_sts->waiting);

  return FALSE;
}

static BC_STATUS
bcmdec_create_shmem (GstBcmDec * bcmdec, int *shmem_id)
{
  int shmid = -1;
  ...
  *shmem_id = shmid;
  ...
  if (shmid == -1) {
    if (errno == EEXIST) {
      GST_DEBUG_OBJECT (bcmdec, "bcmdec_create_shmem:shmem already exists :%d",
          errno);
      shmid = shmget (shmkey, 1024, 0644);
      if (shmid == -1) {
        GST_ERROR_OBJECT (bcmdec, "bcmdec_create_shmem:unable to get shmid :%d",
            errno);
        return BC_STS_INSUFF_RES;
      }
      ...
      if (buf.shm_nattch == 0) {
        ...
        if (shmid == -1) {
          GST_ERROR_OBJECT (bcmdec,
              "bcmdec_create_shmem:unable to get shmid :%d", errno);
          return BC_STS_INSUFF_RES;
        }
        //attach to it
...
static BC_STATUS
bcmdec_get_shmem (GstBcmDec * bcmdec, int shmid, gboolean newmem)
{
  gint ret = 0;
  g_inst_sts = (GLB_INST_STS *) shmat (shmid, (void *) 0, 0);
  if ((intptr_t) g_inst_sts == -1) {
    GST_ERROR_OBJECT (bcmdec, "Unable to open shared memory ...errno = %d",
        errno);
    return BC_STS_ERROR;
  }
...
bcmdec_del_shmem (GstBcmDec * bcmdec)
{
  ...
  if (buf.shm_nattch == 0) {
    ...
    if (shmctl (shmid, IPC_RMID, NULL) != -1) {
      ...
      GST_ERROR_OBJECT (bcmdec,
          "bcmdec_del_shmem:unable to delete shmem segment ...");
    }
  }
...
// For renderer buffer
static void
bcmdec_flush_gstrbuf_queue (GstBcmDec * bcmdec)
{
  GSTBUF_LIST *gst_queue_element = NULL;
...
static void *
bcmdec_process_get_rbuf (void *ctx)
{
  GstBcmDec *bcmdec = (GstBcmDec *) ctx;
  ...
  while (1) {
    ...
    while (bcmdec->streaming && get_buf_start) {
      ...
      if (!gst_queue_element) {
        if (!bcmdec->silent)
          GST_DEBUG_OBJECT (bcmdec, "mbuf full == TRUE %u",
              bcmdec->gst_buf_que_sz);
      ...
      GST_DEBUG_OBJECT (bcmdec,
          "process get rbuf gst_pad_alloc_buffer_and_set_caps ....");
...
static void
bcmdec_ins_padbuf (GstBcmDec * bcmdec, GSTBUF_LIST * gst_queue_element)
{
  pthread_mutex_lock (&bcmdec->gst_padbuf_que_lock);
...
static GSTBUF_LIST *
bcmdec_rem_padbuf (GstBcmDec * bcmdec)
{
  GSTBUF_LIST *temp;
 ...
 */
static gboolean
plugin_init (GstPlugin * bcmdec)
{
  /*
   * debug category for fltering log messages
   */
  GST_DEBUG_CATEGORY_INIT (gst_bcm_dec_debug, "bcmdec", 0,
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR, GST_VERSION_MINOR, bcmdec,
    "Video decoder", plugin_init, VERSION, "LGPL", "bcmdec",
    "http://broadcom.com/")
