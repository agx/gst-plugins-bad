...
gboolean
parse_find_strt_code (Parse * parse, guint8 input_format, guint8 * in_buffer,
...
GetNaluType (Parse * parse, guint8 * pInputBuf, guint32 ulSize, NALU_t * pNalu)
{
  ...
  pNalu->Len =
      (Pos + Rewind) - pNalu->StartcodePrefixLen - nLeadingZero8BitsCount -
      TrailingZero8Bits;

  pNalu->NalUnitType =
      (pInputBuf[nLeadingZero8BitsCount + pNalu->StartcodePrefixLen]) & 0x1f;
